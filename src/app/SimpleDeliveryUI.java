...
public class SimpleDeliveryUI extends JFrame {
    ...
    DefaultTableModel cartModel = new DefaultTableModel(new String[]{"Item","Price"},0);
    ...
    JPanel createLoginPanel(String role){
        ...
        login.addActionListener(e->{user.setText("");pass.setText(""); if(role.equals("Customer")) cardLayout.show(mainPanel,"customerDash"); else if(role.equals("Manager")) cardLayout.show(mainPanel,"managerDash"); else cardLayout.show(mainPanel,"deliveryDash");});
        back.addActionListener(e->{user.setText("");pass.setText("");cardLayout.show(mainPanel,"welcome");});
        return p;
    }
    ...
    JPanel createCustomerPanel(){
        ...
        add.addActionListener(e->{int r=menu.getSelectedRow(); if(r!=-1) cartModel.addRow(new Object[]{menu.getValueAt(r,0),menu.getValueAt(r,1)});});
        del.addActionListener(e->{int r=cart.getSelectedRow(); if(r!=-1) cartModel.removeRow(r);});
        checkout.addActionListener(e->{if(cartModel.getRowCount()>0){sharedOrderModel.addRow(new Object[]{"Customer Order","Pending"});cartModel.setRowCount(0);JOptionPane.showMessageDialog(this,"Order Placed");}else JOptionPane.showMessageDialog(this,"Cart is empty");});
        logout.addActionListener(e->{cartModel.setRowCount(0);cardLayout.show(mainPanel,"welcome");});
        return p;
    }
    ...
    JPanel createManagerPanel(){
        ...
        JPanel south=new JPanel(); south.add(new JLabel("Item"));south.add(item);south.add(new JLabel("Price"));south.add(price);south.add(add);south.add(del);south.add(logout);
        p.add(new JLabel("Manager Dashboard",SwingConstants.CENTER),BorderLayout.NORTH);
        p.add(new JScrollPane(table),BorderLayout.CENTER);
        p.add(south,BorderLayout.SOUTH);
        add.addActionListener(e->{if(!item.getText().trim().isEmpty()&&!price.getText().trim().isEmpty()){sharedMenuModel.addRow(new Object[]{item.getText(),price.getText()});item.setText("");price.setText("");}});
        del.addActionListener(e->{int r=table.getSelectedRow(); if(r!=-1) sharedMenuModel.removeRow(r);});
    ...
    JPanel createDeliveryPanel(){
        ...
        JPanel south=new JPanel(); south.add(done);south.add(logout);
        p.add(new JLabel("Deliveryman Dashboard",SwingConstants.CENTER),BorderLayout.NORTH);
        p.add(new JScrollPane(table),BorderLayout.CENTER);
        p.add(south,BorderLayout.SOUTH);
        done.addActionListener(e->{int r=table.getSelectedRow(); if(r!=-1) sharedOrderModel.setValueAt("Delivered",r,1);});
        logout.addActionListener(e->cardLayout.show(mainPanel,"welcome"));
        return p;
    }
